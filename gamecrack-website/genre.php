<?php
    ...
    //get items
    $sql = "SELECT ga.id, ga.name, ga.slug, c.url, GROUP_CONCAT(DISTINCT(ge.name)) AS genres, GROUP_CONCAT(DISTINCT(ge.slug)) AS genres_slug
    FROM game ga JOIN game_genre gg
    ON ga.id = gg.game_id
    JOIN genres ge
    ON ge.id = gg.genre_id
    ...
    ON ga.id = c.game_id
    GROUP BY ga.id
    HAVING genres_slug LIKE '%$genre_slug%'
...
<body>
    ...
    <ul class="pagination">
        <li class="page-item <?= ($page <= 1) ? 'disabled' : ''; ?>">
            <a class="page-link" <?= ($page > 1) ? 'href=?slug='.$genre_slug.'&page='.$start : ''; ?>>
                <i class="fa-solid fa-angles-left"></i>
            </a>
        </li>
        <li class="page-item <?= ($page <= 1) ? 'disabled' : ''; ?>">
            <a class="page-link" <?= ($page > 1) ? 'href=?slug='.$genre_slug.'&page='.$prev : ''; ?>>
                <i class="fa-solid fa-angle-left"></i>
            </a>
        </li>
        <?php for ($count = $page; $count <= $total_pages; $count++) { ?>
                <li class="page-item <?= ($count <= $count_lmt) ? '' : 'disabled'; ?>">
                    <a class="page-link" href="?slug=<?= $genre_slug ?>&page=<?= $count; ?>"><?= $count; ?></a>
                </li>
        <?php } ?>
        <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : ''; ?>">
            <a class="page-link" <?= ($page < $total_pages) ? 'href=?slug='.$genre_slug.'&page='.$next : ''; ?>>
                <i class="fa-solid fa-angle-right"></i>
            </a>
        </li>
        <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : ''; ?>">
            <a class="page-link" <?= ($page < $total_pages) ? 'href=?slug='.$genre_slug.'&page='.$end : ''; ?>>
