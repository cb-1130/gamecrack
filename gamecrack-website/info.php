<?php
    include_once('conn.php');

    $idsql = "SELECT GROUP_CONCAT(id) AS id FROM game;";
    $res_id = mysqli_query($con, $idsql) or die(mysqli_error($con));
    $row_id = mysqli_fetch_array($res_id);
    $id = $row_id['id'];
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link type="text/css" rel="stylesheet" href="css/info.css" />
    <script src="https://kit.fontawesome.com/e534f669f8.js" crossorigin="anonymous"></script>
    <script type="text/javascript">

        function randomGame() {
            const random = document.getElementById('random');
            const id = [<?= $id ?>];
            const num = Math.floor(Math.random() * id.length);
            random.setAttribute('href', 'game.php?id=' + id[num]);
        }
    </script>
    <title>Info » GameCrack</title>
</head>
<body>

    <!-- Navigation Bar-->
    <div class="navbar">
        <div class="navbar-left">
            <a class="logo" href="index.php">
                <img alt="logo" src="data:image/png;base64," />
            </a>
            <form class="searchbar" action="search.php" method="GET">
                <input type="text" name="query" />
                <button class="fa fa-search search" type="submit"></button>
            </form>
            <ul class="menu">
                <li onclick="randomGame()"><a id="random" href=""> Random </a></li>
                <li><a href="browse.php"> Browse </a></li>
                <li><a href="info.php"> Info </a></li>
            </ul>
        </div>
    </div>

    <!-- Content -->
    <div class="content">
        <div id="info-container" class="container">
            <section>
                <h2> Features </h2>
                <ul>
                    <li> No, we do not provide cracks or illegal files. </li>
                    <li> No, we do not help or encourage users to download or otherwise obtain any copyrighted material. </li>
                    <li> Yes, we are an information hub to games that have been cracked / uncracked by the scene. </li>
                </ul>
            </section>
            <section>
                <h2> Accounts </h2>
                <ul>
                    <li> Account creation is sadly not available and will not be implemented for the foreseeable future. </li>
                </ul>
            </section>
            <section>
                <h2> Credits </h2>
                <ul>
                    <li> All of the information about the games and everything is provided by IGDB (https://api-docs.igdb.com/). </li>
                    <li> All trademarks are property of their respective owners in the US and other countries. </li>
                </ul>
            </section>
            <section>
                <h2> Want to get in touch? </h2>
                <ul>
                    <li> Email: <a href="mailto:hey@gamecrack.com"> hey@gamecrack.com </a> </li>
                    <li> Reddit: <a href="https://reddit.com/r/GameCrack"> r/GameCrack </a> </li>
                    <li> Twitter: <a href="https://twitter.com/GameCrack"> @GameCrackOfficial </a> </li>
                </ul>
            </section>
            <section id="thanks">
                Thanks for contributing and supporting the site!
                <br>
                This is a college group project, pardon if it is lacking.
                <br>
                <i class="fa fa-heart color-icon"></i> Love
                <br>
                -Group 7 (GameCrack)
            </section>
        </div>
    </div>

</body>
</html>